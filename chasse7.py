    ...
    if new_indice == 1:
        ...
        if text is None or any(character.isdigit() for character in text): # Si ca retourne pas une bonne direction, ré essayer avec une autre prise de vue du screen (car certaines directions sont sur 2 lignes et d'autres 1)
            ...
            if indice_fleche_text_check(text):
                input("Aucune direction trouvée.")
...
def indice_fleche_text_check(text): # check le text retourné en lisant un indice ou une direction
    ...
    if any(character.isdigit() for character in text) or not matches or not text or 'Départ' in text or text is None:
        return True
    else:
        return False
...
def move_to_destination(destination_x, destination_y):
    ...
    while(actual_x != destination_x or actual_y != destination_y):
        ...
        actual_x, actual_y = get_coordinates()
        if(stuck_test > 120):
            input("Personnage bloqué lors d'un déplacement.")
            # ICI METTRE UN TRUC QUI FAIT QUE CA QUITTE LA CHASSE POUR EN RELANCER  UNE AUTRE, NORMALEMENT LE PROGRAMME TOURNE H24 COMME CA
    time.sleep(0.3)
...
def move_to_phorreur(direction):
    ...
    for i in range(11):
        ...
        if x is None or y is None:
            input("Erreur dans la récupération des coordonnées. (get_coordinates)")
            return
...
def custom_autopilot(destination_x, destination_y):
    ...
    while(actual_x != destination_x or actual_y != destination_y):
        ...
        actual_x, actual_y = get_coordinates()
        if(stuck_test > 120):
            input("Personnage bloqué lors de l'autopilotage.")
    time.sleep(0.5)
    check_and_fill_coordinates()
    time.sleep(0.5)
    letsgo()
...
def check_zone_image(confidence, image_name, x, y, x2, y2):
    screenshot = pyautogui.screenshot()
    region = screenshot.crop((x, y, x2, y2))
    region.save('check_zone_image.png')
...
keyboard.add_hotkey('*', spot_phorreur)
